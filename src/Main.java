...
--- Критерии оценки:
        ...
        – Написанный код отрабатывает условия задачи и изменение значений переменных,
          выводит в консоль корректный результат
        – Правила синтаксиса и пунктуации соблюдены.
 */
        int age = 25;
        int salary = 60_000;
...
и их возможность выплачивать кредит. Для того, чтобы вывести предварительное решение, нам необходимо использовать
 ...
 */
        ...
        double creditRateMonth = creditRate / 12 / 100;
        int totalMonths = timeOfCredit * 12;
        double pay = wantedSum * ((creditRateMonth * (Math.pow((1 + creditRateMonth), totalMonths)) / ((Math.pow((1 + creditRateMonth), totalMonths)) - 1)));
        System.out.print("Максимальный платеж при ЗП " + salary + " руб равен " + maxPay + " руб. Платеж по кредиту " + (Math.ceil(pay * Math.pow(10, 2)) / Math.pow(10, 2)) + " руб. ");
        if (maxPay > pay) {
